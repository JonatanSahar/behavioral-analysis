...
def get_subject_data(subject_id="p33"):
    ...
    session_names = zip(["day_1_pre", "day_1_post", "day_2_pre", "day_2_post"], ["-1-1_results.csv",  "-1-2_results.csv", "-2-1_results.csv",  "-2-2_results.csv"])
    ...
    df_all_subjects = pd.concat([d_all_session_data[name]["response score"]
                                          for name, _ in session_names], axis='columns')
